                        ...
                        <div id="search-container" class="transition-all duration-300"
                            ...
                            <form method="GET" action="{{ route('user.index') }}" id="search-form"
                                ...
                                <div class="flex items-center">
                                    ...
                                    @if (request('q'))
                                        <a href="{{ route('user.index', request()->except('q')) }}"
                    ...
                    <form method="GET" action="{{ route('user.index') }}">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 items-end">
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                @if ($hasFilter)
                                    <a href="{{ route('user.index', request()->only('q')) }}"
...
@push('scripts')
    <script>
        ...
        document.getElementById('confirm-delete-btn').addEventListener('click', function() {
            if (deleteUuid) submitAction('/user/' + deleteUuid, 'DELETE');
        });
        ...
        document.getElementById('confirm-verify-btn').addEventListener('click', function() {
            if (verifyUuid) submitAction('{{ route('user.verify-email', ':id') }}'.replace(':id', verifyUuid),
                'POST');
        });
        ...
        document.getElementById('confirm-status-btn').addEventListener('click', function() {
            if (statusUuid) submitAction('{{ route('user.toggle-status', ':id') }}'.replace(':id', statusUuid),
                'POST');
        });
