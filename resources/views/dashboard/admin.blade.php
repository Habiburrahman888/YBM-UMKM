        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 hover:shadow-md transition-shadow">
                <div class="flex items-center justify-between mb-3">
                    ...
                    @if ($usersGrowth > 0)
                        <span class="text-xs font-semibold text-green-600 bg-green-50 px-2 py-1 rounded-full">+{{ $usersGrowth }}%</span>
                    @elseif($usersGrowth < 0)
                        <span class="text-xs font-semibold text-red-600 bg-red-50 px-2 py-1 rounded-full">{{ $usersGrowth }}%</span>
                    @endif
                </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 hover:shadow-md transition-shadow">
                <div class="flex items-center justify-between mb-3">
                    ...
                    @if ($unitsGrowth > 0)
                        <span class="text-xs font-semibold text-green-600 bg-green-50 px-2 py-1 rounded-full">+{{ $unitsGrowth }}%</span>
                    @elseif($unitsGrowth < 0)
                        <span class="text-xs font-semibold text-red-600 bg-red-50 px-2 py-1 rounded-full">{{ $unitsGrowth }}%</span>
                    @endif
                </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 hover:shadow-md transition-shadow">
                <div class="flex items-center justify-between mb-3">
                    ...
                    @if ($umkmGrowth > 0)
                        <span class="text-xs font-semibold text-green-600 bg-green-50 px-2 py-1 rounded-full">+{{ $umkmGrowth }}%</span>
                    @elseif($umkmGrowth < 0)
                        <span class="text-xs font-semibold text-red-600 bg-red-50 px-2 py-1 rounded-full">{{ $umkmGrowth }}%</span>
                    @endif
                </div>
                    ...
                    @forelse($topProvinsi as $index => $provinsi)
                        <div>
                            <div class="flex items-center justify-between mb-1">
                                ...
                                <span class="text-sm font-bold text-gray-900">{{ number_format($provinsi['total'] ?? 0) }} UMKM</span>
                            </div>
        ...
        <div class="grid lg:grid-cols-3 gap-6 mb-6">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                ...
                <div class="space-y-3 max-h-96 overflow-y-auto custom-scrollbar">
                    @forelse($unitList as $unit)
                        <div class="p-4 bg-gradient-to-br from-purple-50 to-pink-50 rounded-lg hover:shadow-md transition-all border border-purple-100">
                            ...
                            <p class="text-xs text-gray-600 mb-2">Admin: {{ optional($unit->user)->name ?? 'Belum ada admin' }}</p>
                            <div class="flex items-center text-xs text-gray-500">
                                <svg class="w-3 h-3 mr-1 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                                    <path fill-rule="evenodd" d="M5.05 4.05a7 7 0 119.9 9.9L10 18.9l-4.95-4.95a7 7 0 010-9.9zM10 11a2 2 0 100-4 2 2 0 000 4z" clip-rule="evenodd" />
                                </svg>
                                <span class="truncate">{{ $unit->kota_nama ?? ($unit->alamat ?? 'Lokasi belum diset') }}</span>
                            </div>
                        </div>
                    @empty
                        <div class="text-center py-12">
                ...
                <div class="space-y-4 max-h-96 overflow-y-auto custom-scrollbar">
                    @forelse($aktivitasTerbaru as $aktivitas)
                        <div class="flex gap-3">
                            <div class="w-2 h-2 bg-{{ $aktivitas['color'] ?? 'gray' }}-500 rounded-full mt-2 flex-shrink-0"></div>
                            <div class="flex-1 min-w-0">
                                <p class="text-sm font-medium text-gray-900">{{ $aktivitas['title'] ?? 'Aktivitas' }}</p>
                                <p class="text-xs text-gray-600 mt-1 break-words">{{ $aktivitas['description'] ?? '' }}</p>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <a href="{{ route('umkm.index') }}" class="text-xs text-blue-600 hover:text-blue-700 font-medium transition-colors">
                        Lihat Semua →
                    </a>
                </div>
                <div class="space-y-3 max-h-96 overflow-y-auto custom-scrollbar">
                    @forelse($umkmTerbaru as $umkm)
                        <div class="p-4 bg-gradient-to-br from-blue-50 to-indigo-50 rounded-lg hover:shadow-md transition-all border border-blue-100">
                            <div class="flex items-center gap-2 mb-2 flex-wrap">
                            ...
                            <p class="text-sm font-semibold text-gray-900 mb-1 truncate" title="{{ $umkm->nama_usaha ?? '-' }}">
                                {{ $umkm->nama_usaha ?? '-' }}
                            </p>
