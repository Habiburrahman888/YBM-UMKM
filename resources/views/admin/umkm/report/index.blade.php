        ...
        <div class="bg-white rounded-xl sm:rounded-2xl shadow-card border border-gray-100 overflow-hidden animate-slide-up">
            ...
            <div class="px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-200">
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        {{-- Tombol Download PDF --}}
                        <a href="{{ route('admin.report.all') }}?{{ http_build_query(request()->only(['tahun', 'tahun_berdiri', 'unit_id'])) }}"
                            target="_blank"
            ...
            <div id="filter-section"
                ...
                <div class="px-4 sm:px-6 py-4 bg-gray-50">
                    <form method="GET" action="{{ route('admin.report.preview') }}">
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 items-end">
                            ...
                            <div>
                                ...
                                <select name="tahun_berdiri"
                                    class="block w-full text-sm border border-gray-300 rounded-lg px-3 py-2 bg-white focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary">
                                    <option value="">Semua Tahun</option>
                                    @foreach ($tahunBerdiriList as $tahun)
                                        <option value="{{ $tahun }}"
                                            {{ request('tahun_berdiri') == $tahun ? 'selected' : '' }}>{{ $tahun }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    @foreach ($items as $i => $umkm)
                                        <tr class="hover:bg-gray-50 transition-colors">
                                            ...
                                            <td class="px-4 py-4">
                                                <p class="text-xs text-gray-500 mb-0.5">{{ $umkm->kategori?->nama ?? '—' }}</p>
                                                <p class="text-sm font-medium text-gray-900 leading-snug">{{ $umkm->nama_usaha }}</p>
                                                <p class="text-xs text-gray-400 mt-0.5">{{ $umkm->nama_pemilik }}</p>
                                            </td>
                                            <td class="px-4 py-4 min-w-[200px]">
                                                @if ($umkm->modalUmkm->isNotEmpty())
                                                    <div class="flex flex-wrap gap-1">
                                                        @foreach ($umkm->modalUmkm as $modal_item)
                                                            <span class="inline-flex items-center px-2 py-0.5 rounded-md bg-gray-100 text-gray-600 text-[11px] font-medium" title="{{ $modal_item->nama_item }} - Rp {{ number_format($modal_item->nilai_modal, 0, ',', '.') }}">
                                            ...
                                            <td class="px-4 py-4">
                                                @if ($umkm->city || $umkm->province)
                                                    <div class="text-sm font-medium text-gray-700">{{ $umkm->city?->name ?? '—' }}</div>
                                                    @if ($umkm->province)
                                                        <div class="text-xs text-gray-400 mt-0.5 uppercase tracking-wide">{{ $umkm->province->name }}</div>
                                                    @endif
                                                @else
                                                    <span class="text-sm text-gray-400">—</span>
                                                @endif
                                            </td>
                                            <td class="px-4 py-4 text-sm text-gray-600 whitespace-nowrap">{{ $umkm->tanggal_bergabung?->translatedFormat('d M Y') ?? '—' }}</td>
                                            <td class="px-4 py-4 text-center text-sm text-gray-700">{{ $umkm->tahun_berdiri ?? '—' }}</td>
                                            <td class="px-4 py-4 text-right text-sm font-medium text-gray-900">{{ $umkm->total_modal }}</td>
                                            <td class="px-4 py-4">
                                            ...
                                            <td class="px-4 sm:px-6 py-4 text-center">
                                                <a href="{{ route('admin.report.single', $umkm->uuid) }}" target="_blank" class="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-red-50 hover:bg-red-100 text-red-600 transition-colors">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach ($umkmList as $i => $item)
                                    <tr class="hover:bg-gray-50 transition-colors">
                                        ...
                                        <td class="px-4 sm:px-6 py-4 text-center">
                                            <a href="{{ route('admin.report.single', $item->uuid) }}" target="_blank" class="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-red-50 hover:bg-red-100 text-red-600 transition-colors">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                                                </svg>
                                            </a>
                        ...
                        <div>
                            ...
                            <div class="px-4 py-4 hover:bg-gray-50/60 transition-colors cursor-pointer"
                                    ...
                                    <div class="flex items-start gap-3 flex-1 min-w-0">
                                        ...
                                        {{-- Chevron --}}
                                        <svg id="icon-mob-report-{{ $umkm->uuid }}"
                                            class="w-3.5 h-3.5 flex-shrink-0 mt-1.5 text-gray-300 transition-transform duration-200"
                                        ...
                                        {{-- Avatar / Inisial --}}
                                        @if ($umkm->logo_umkm)
                                            <img src="{{ Storage::url($umkm->logo_umkm) }}"
                                                alt="{{ $umkm->nama_usaha }}"
                                        ...
                                        @else
                                            <div
                                                ...
                                                <span
                                                    class="text-sm font-bold text-primary">{{ strtoupper(substr($umkm->nama_usaha, 0, 1)) }}</span>
                                            </div>
                                        @endif
                                        ...
                                        <div class="flex-1 min-w-0">
                                            ...
                                            @if ($umkm->city || $umkm->province)
                                                <p class="text-xs text-gray-400 mt-0.5">
                                                    {{ collect([$umkm->city?->name, $umkm->province?->name])->filter()->implode(', ') }}
                                                </p>
                                            @endif
                                        </div>
                                    </div>
                                    ...
                                    {{-- Tombol PDF --}}
                                    <a href="{{ route('admin.report.single', $umkm->uuid) }}" target="_blank"
                            ...
                            {{-- Accordion Detail --}}
                            <div id="detail-mob-report-{{ $umkm->uuid }}" class="hidden bg-gray-50 px-4 pb-4">
                                <div class="ml-6 pt-3 border-t border-gray-100 space-y-2.5">
                                    @if ($umkm->kategori)
                                        <div>
                                            <span class="text-xs text-gray-400">Kategori</span>
                                            <p class="text-sm text-gray-700 mt-0.5">{{ $umkm->kategori->nama }}</p>
                                        </div>
                                    @endif
                                    @if ($umkm->tanggal_bergabung)
                                        <div>
                                            <span class="text-xs text-gray-400">Tanggal Bergabung</span>
                                            <p class="text-sm text-gray-700 mt-0.5">
                                                {{ $umkm->tanggal_bergabung->translatedFormat('d F Y') }}</p>
                                        </div>
                                    @endif
                                    @if ($umkm->tahun_berdiri)
                                        <div>
                                            <span class="text-xs text-gray-400">Tahun Berdiri</span>
                                            <p class="text-sm text-gray-700 mt-0.5">{{ $umkm->tahun_berdiri }}</p>
                                        </div>
                                    @endif
                                    @if ($umkm->modalUmkm->isNotEmpty())
                                        <div>
                                            ...
                                            <p class="text-sm font-medium text-gray-900 mt-1">{{ $umkm->total_modal }}</p>
                                        </div>
                                    @endif
                                    @if (auth()->user()->role === 'admin' && $umkm->unit)
                                        <div>
                                            <span class="text-xs text-gray-400">Unit</span>
                                            <p class="text-sm text-gray-700 mt-0.5">{{ $umkm->unit->nama }}</p>
                                        </div>
                                    @endif
                                </div>
                            </div>
