    ...
    <div class="space-y-4 sm:space-y-6">
        <div class="bg-white rounded-xl sm:rounded-2xl shadow-card border border-gray-100 overflow-hidden animate-slide-up">
            <div class="px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-200">
                <h2 class="text-base sm:text-lg font-semibold text-gray-900">Edit Kategori</h2>
                <p class="text-xs sm:text-sm text-gray-500 mt-1">Perbarui informasi kategori</p>
            </div>

            <form action="{{ route('admin.kategori.update', $kategori->uuid) }}" method="POST" class="p-4 sm:p-6">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-4 sm:space-y-5">
                    ...
                    <div>
                        ...
                        <input type="text" name="nama" id="nama" value="{{ old('nama', $kategori->nama) }}" autofocus
                    ...
                    <div class="flex justify-end">
                        <span class="text-xs text-gray-500">
                            <span id="char-count">{{ strlen(old('nama', $kategori->nama)) }}</span> / 255 karakter
                        </span>
                    </div>
                </div>
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Trigger input event to set initial character count color
