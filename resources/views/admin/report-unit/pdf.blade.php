...
<body>
    ...
    <div class="header">
        <div class="header-left">
            <h1>{{ $title }}</h1>
            <p>{{ $isSingleUnit ? 'Data UMKM binaan pada unit ' . ($singleUnit ? $singleUnit->nama_unit : 'pusat') : 'Rekapitulasi data unit dan pendampingan UMKM YBM.' }}
            </p>
        </div>
    ...
    @foreach ($unitList as $unit)
        ...
        <div class="unit-section">
            <div class="unit-header">
                <div class="unit-title text-uppercase">UNIT: {{ strtoupper($unit?->nama_unit ?? 'PUSAT / TANPA UNIT') }}
                </div>
                <div class="unit-total">Total Modal Unit: Rp
                    {{ number_format($items->sum(fn($u) => $u->modalUmkm->sum('nilai_modal')), 0, ',', '.') }}</div>
            </div>
            <table>
                <thead>
                    <tr>
                ...
                <tbody>
                    @forelse ($items as $index => $item)
                        <tr>
                            ...
                            <td>
                                ...
                                <div class="owner">{{ $item->kategori->nama ?? '-' }} | Berdiri
                                    {{ $item->tahun_berdiri ?? '-' }}</div>
                            </td>
                            <td>
                                ...
                                @else
                                    <span style="color: #cbd5e1; font-style: italic; font-size: 7pt;">- belum ada data
                                        -</span>
                                @endif
                            </td>
                            <td class="status">
                    ...
                    @empty
                        <tr>
                            <td colspan="5"
                                style="text-align: center; color: #94a3b8; font-style: italic; padding: 15px;">
                                - Belum ada data UMKM untuk unit ini -
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    <table style="margin-top: 10px; background: #0f172a; color: #fff; border: 0;">
        <tr>
            ...
            <td
                style="padding: 12px; width: 120px; font-weight: bold; font-size: 10pt; text-align: right; border-left: 1px solid #334155;">
