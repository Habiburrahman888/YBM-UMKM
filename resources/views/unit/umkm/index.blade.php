        ...
        <div class="bg-white rounded-xl sm:rounded-2xl shadow-card overflow-hidden animate-slide-up">
                ...
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <div id="search-container" class="transition-all duration-300"
                            ...
                            @php $umkmRoute = ($permissions['userRole'] === 'admin') ? route('admin.umkm.index') : route('umkm.index'); @endphp
                            <form method="GET" action="{{ $umkmRoute }}" id="search-form"
                                ...
                                <div class="flex items-center">
                                    ...
                                    @if (request('q'))
                                        <a href="{{ $umkmRoute . '?' . http_build_query(request()->except('q')) }}"
            ...
            <div id="filter-section"
                ...
                <div class="px-4 sm:px-6 py-4 bg-gray-50">
                    <form method="GET" action="{{ $umkmRoute }}">
                        @if (request('q'))
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-3 items-end">
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                @if ($hasFilter)
                                    <a href="{{ $umkmRoute . (request('q') ? '?q=' . request('q') : '') }}"
                                    ...
                                    @foreach ($group as $item)
                                        ...
                                        <tr class="{{ $rowClass }} hover:bg-gray-50 transition-colors cursor-pointer"
                                            ...
                                            <td class="px-4 py-4 min-w-[220px] overflow-visible">
                                                @include('unit.umkm.partials.modal_usaha_column', [
                                                    'item' => $item,
                                                ])
                                            </td>
                                            ...
                                            @if (in_array(auth()->user()->role, ['admin', 'unit']))
                                                <td class="px-4 sm:px-6 py-4 text-right"
                                                    ...
                                                    @include('unit.umkm.partials.actions_dropdown', [
                                                        'item' => $item,
                                        ...
                                        <tr id="detail-{{ $item->uuid }}"
                                            ...
                                            <td colspan="{{ in_array(auth()->user()->role, ['admin', 'unit']) ? 5 : 4 }}"
                                                ...
                                                @include('unit.umkm.partials.detail_accordion', [
                                                    'item' => $item,
                                                ])
                                            </td>
                                        </tr>
                            ...
                            @else
                                ...
                                @foreach ($umkmList as $item)
                                    ...
                                    <tr class="hover:bg-gray-50 transition-colors cursor-pointer"
                                        ...
                                        {{-- Modal Usaha --}}
                                        <td class="px-4 py-4 min-w-[220px] overflow-visible">
                                            @include('unit.umkm.partials.modal_usaha_column', ['item' => $item])
                                        </td>
                                    ...
                                    <tr id="detail-{{ $item->uuid }}" class="hidden bg-gray-50">
                                        <td colspan="{{ in_array(auth()->user()->role, ['admin', 'unit']) ? 5 : 4 }}"
                                            class="px-4 sm:px-6 py-3">
                                            @include('unit.umkm.partials.detail_accordion', ['item' => $item])
                                        </td>
                                    </tr>
                                @endforeach
            ...
            @else
                ...
                : 'Mulai tambahkan data UMKM untuk mengelola usaha mikro, kecil, dan menengah.' }}
                            ...
                            @if ($permissions['userRole'] !== 'admin' && !request()->hasAny(['q', 'status', 'kategori_id', 'province_code', 'city_code']))
                                <a href="{{ route('umkm.create') }}"
...
@push('scripts')
    <script>
        ...
        document.getElementById('confirm-delete-btn').addEventListener('click', function() {
            if (deleteUuid) {
                const deletePath = '{{ $permissions['userRole'] === 'admin' ? '/admin/umkm/' : '/umkm/' }}';
                submitAction(deletePath + deleteUuid, 'DELETE');
            }
        });
